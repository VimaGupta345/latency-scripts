...
# Run all 4 models x 4 benchmarks SEQUENTIALLY — FULL DATASETS (no limit)
# Usage: zsh run_all_sequential_full.sh
# Each model runs alone on its required GPUs.
# Passes limit=0 which tells lm_eval_online_serve.py to omit --limit (run all examples).

export TMP_HOME=/data/vgupta345/prowl_related_data/prowl-open-source

LOGDIR="${TMP_HOME}/results/logs"
mkdir -p "${LOGDIR}"
LOGFILE="${LOGDIR}/all_sequential_full_$(date +%Y%m%d-%H%M%S).log"
...
echo "========================================="
echo "SEQUENTIAL FULL-DATASET RUN: 4 models x 4 benchmarks x 2 configs"
echo "No --limit flag — runs all examples per benchmark"
echo "Log: ${LOGFILE}"
echo "Started: $(date)"
echo "========================================="
...
echo ""
echo "========================================="
echo "ALL SEQUENTIAL FULL-DATASET RUNS COMPLETED"
echo "Finished: $(date)"
